...
parser = argparse.ArgumentParser('jodie')

# select dataset and training mode
# parser.add_argument('--dataset', default='wikipedia', type=str, help='Network name', choices=['wikipedia', 'mooc', 'reddit', 'lastfm', 'douban_movie', 'ml1m', 'video', 'tools', 'yoochoosebuy', 'videogames', 'garden'],)
parser.add_argument('--dataset', default='wikipedia', type=str, help='Network name', choices=['Beauty_5', 'douban_movie', 'ml1m', 'CDs_and_Vinyl_5', 'Sports_and_Outdoors_5', 'Video_Games_5', 'Douban_Movie'],)
parser.add_argument('--model', default='jodie',type=str, help="Model name")
...
'''HIGH_MODE = "pct"      # "k" 또는 "pct"
HIGH_THR  = 90        # mode="k"면 k값, mode="pct"면 백분위(예: 90)
HighUsers, LowUsers, used_tau, pair_count = build_high_low_users_by_overlap(
    user_sequence_id, item_sequence_id, train_end_idx, num_users,
    mode=HIGH_MODE, threshold=HIGH_THR
)
print(f"[Split] mode={HIGH_MODE}, thr={HIGH_THR}, "
      f"|HighUsers|={len(HighUsers)}, |LowUsers|={len(LowUsers)}")'''
...
# with trange(args.epochs, desc="Training", ascii=True) as progress_bar1:
for ep in range(args.epochs):
    ...
    # with trange(train_end_idx, ascii=True) as progress_bar2:
...
test_mrr, test_rec1, test_rec5, test_rec10, test_rec20, test_pre1, test_pre5, test_pre10, test_pre20, test_ndcg1, test_ndcg5, test_ndcg10, test_ndcg20, inference_time, test_peak_memory_mb = eval_one_epoch(args, model, optimizer, MSELoss, crossEntropyLoss, weight, user_sequence_id, item_sequence_id, feature_sequence, item_feat_flag, user_feat_flag, item_feature, user_feature, 
                   user_timediffs_sequence, item_timediffs_sequence, timestamp_sequence, user_previous_itemid_sequence, 
                   user_embeddings, item_embeddings, user_embeddings_static, item_embeddings_static, user_embeddings_timeseries, item_embeddings_timeseries,
                   test_start_idx, test_end_idx) #, filter_users=HighUsers)

# logger.info("[TEST-HIGH]")
...
'''test_mrr, test_rec1, test_rec5, test_rec10, test_rec20, test_pre1, test_pre5, test_pre10, test_pre20, test_ndcg1, test_ndcg5, test_ndcg10, test_ndcg20, inference_time = eval_one_epoch(args, model, optimizer, MSELoss, crossEntropyLoss, weight, user_sequence_id, item_sequence_id, feature_sequence, item_feat_flag, user_feat_flag, item_feature, user_feature, 
                   user_timediffs_sequence, item_timediffs_sequence, timestamp_sequence, user_previous_itemid_sequence, 
                   user_embeddings, item_embeddings, user_embeddings_static, item_embeddings_static, user_embeddings_timeseries, item_embeddings_timeseries,
                   validation_start_idx, test_start_idx, filter_users=LowUsers)

logger.info("[TEST-LOW]")
logger.info('Best epoch: {}'.format(early_stopper.best_epoch))
logger.info('test MRR:\t{:.4f}'.format(test_mrr))
logger.info('test REC:\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}'.format(test_rec1, test_rec5, test_rec10, test_rec20))
logger.info('test PRE:\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}'.format(test_pre1, test_pre5, test_pre10, test_pre20))
