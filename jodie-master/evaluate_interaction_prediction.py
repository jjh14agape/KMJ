...
parser.add_argument('--dataset', default='lastfm', type=str, help='Network name')
...
parser.add_argument('--embedding_dim', default=32, type=int, help='Number of dimensions')
parser.add_argument('--train_proportion', default=0.8, type=float, help='Proportion of training interactions')
parser.add_argument('--state_change', default='False', type=str, help='True if training with state change of users in addition to the next interaction prediction. False otherwise. By default, set to True. MUST BE THE SAME AS THE ONE USED IN TRAINING.') 
args = parser.parse_args()

print(args) 
args.datapath = "D:/KMJ/DyRec/dataset/%s/%s.csv" % (args.dataset, args.dataset)
...
output_fname = "results/interaction_prediction_%s.txt" % args.dataset
