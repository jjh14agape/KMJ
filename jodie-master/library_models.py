...
def load_model(model, optimizer, args, path):
    ...
    # filename = PATH + "saved_models/%s/checkpoint_%s_epoch%d_size%d_lr%s_wd%s_seed%d.pth.tar" % (args.dataset, args.model, epoch, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed)
    # # filename = PATH + "saved_models/%s/checkpoint_%s_epoch%d_size%d_lr%f_wd%f_seed%d.pth.tar" % (args.dataset, args.model, epoch, args.embedding_dim, args.learning_rate, args.weight_decay, args.seed)
