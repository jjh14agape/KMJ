...
parser.add_argument('--dataset', default='wikipedia', type=str, help='Network name', choices=['wikipedia', 'mooc', 'reddit', 'lastfm'],)
...
if args.online_test:
    ...
    output_fname = "results/prediction_on_test_%s_size%d_lr%s_wd%s_seed%d_online_test.txt" % (args.dataset, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed)
else:
    print("Offline test mode.")
    # CHECK IF THE OUTPUT OF THE EPOCH IS ALREADY PROCESSED. IF SO, MOVE ON.
    output_fname = "results/prediction_on_test_%s_size%d_lr%s_wd%s_seed%d.txt" % (args.dataset, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed)
...
for epp in range(0, args.epochs):
    ...
    # LOAD THE MODEL
    model, optimizer, user_embeddings_dystat, item_embeddings_dystat, user_embeddings_timeseries, item_embeddings_timeseries, train_end_idx_training = load_model(model, optimizer, args, epp)
