...
#     pbar.close()
#     if fast_eval:
#         val_mrr, test_mrr = rollout_evaluate_fast(
#             model, valid_dl, test_dl, last_xu.detach(), last_xi.detach()
#         )
...
def train_one_epoch(model, optimizer, train_dl, delta_coef=1e-5, tbptt_len=20,
    ...
    for i, batch in enumerate(pbar):
        t, dt, adj, i2u_adj, u2i_adj, users, items = batch
        step_loss, delta_norm, last_xu, last_xi, *_ = model.propagate_update_loss(
            adj, dt, last_xu, last_xi, i2u_adj, u2i_adj, users, items
        )
    ...
    if fast_eval:
        valid_xu, valid_xi, valid_ranks = rollout(
            valid_dl, model, last_xu.detach(), last_xi.detach()
        )
        ...
        return val_mrr, val_rec10, val_pre10, val_ndcg10
    else:
        raise NotImplementedError

def rollout_evaluate_valid(model, valid_dl, train_xu, train_xi):
    valid_xu, valid_xi, valid_ranks = rollout(valid_dl, model, train_xu, train_xi)

    val_mrr = mrr(valid_ranks)
    val_recall10 = recall_at_k(valid_ranks, 10)

    print(f"------- Valid MRR: {val_mrr:.4f} Recall@10: {val_recall10:.4f}")

    return val_mrr

def evaluate_test(model, test_dl):
    init_xu, init_xi = model.get_init_states()
    _, _, test_ranks = rollout(test_dl, model, init_xu, init_xi)
    ...
    return test_mrr, test_rec10, test_pre10, test_ndcg10
