...
from tqdm.std import tqdm, trange # tqdm_notebook, tnrange
import csv
...
class DGCF(nn.Module):
    def __init__(self, args, num_features, num_users, num_items):
        ...
        self.length = args.sample_length
...
def save_model(model, optimizer, args, epoch, user_embeddings, item_embeddings, train_end_idx, user_adj, item_adj,
    ...
    # else:
    #     directory = os.path.join(path, 'saved_models_%d/%s/' % (args.sample_length, args.dataset))
    
    # if not os.path.exists(directory):
    #     os.makedirs(directory)

    # filename = os.path.join(directory, "checkpoint_%s_epoch%d_size%d_lr%s_wd%s_seed%d.pth.tar" % (args.model, epoch, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed))
    '''if model.adj:
...
def load_model(model, optimizer, args, path):
    ...
    #     dic = 'saved_models/'
    # else:
    #     dic = 'saved_models_%s/' % args.sample_length
    # filename = PATH + dic + "%s/checkpoint_%s_epoch%d_size%d_lr%s_wd%s_seed%d.pth.tar" % (args.dataset, args.model, epoch, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed)
    '''if model.adj:
        if args.l2u == 1.0 and args.l2i == 1.0:
            filename = PATH + dic +"%s/adj_checkpoint.%s.%s.ep%d.tp%.1f.pth.tar" % (
                args.dataset, modelname, model.method, epoch, args.train_proportion)
        else:
            filename = PATH + dic + "%s/adj_checkpoint.%s.%s.user%.1f.item%.1f.ep%d.tp%.1f.pth.tar" % (
            args.dataset, modelname, model.method, args.l2u, args.l2i, epoch, args.train_proportion)
    elif model.no_zero:
        if args.l2u == 1.0 and args.l2i == 1.0:
            filename = PATH + dic + "%s/adj_checkpoint.%s.%s.%s.ep%d.tp%.1f.pth.tar" % (
                args.dataset, modelname, model.method, 'no_zero', epoch, args.train_proportion)
        else:
            filename = PATH + dic + "%s/adj_checkpoint.%s.%s.%s.user%.1f.item%.1f.ep%d.tp%.1f.pth.tar" % (
                args.dataset, modelname, model.method, 'no_zero', args.l2u, args.l2i, epoch, args.train_proportion)
    elif model.no_first:
        if args.l2u == 1.0 and args.l2i == 1.0:
            filename = PATH + dic + "%s/adj_checkpoint.%s.%s.%s.ep%d.tp%.1f.pth.tar" % (
                args.dataset, modelname, model.method, 'no_first', epoch, args.train_proportion)
        else:
            filename = PATH + dic + "%s/adj_checkpoint.%s.%s.%s.user%.1f.item%.1f.ep%d.tp%.1f.pth.tar" % (
                args.dataset, modelname, model.method, 'no_first', args.l2u, args.l2i, epoch, args.train_proportion)
    else:
        if args.l2u == 1.0 and args.l2i == 1.0:
            filename = PATH + dic + "%s/checkpoint.%s.ep%d.tp%.1f.pth.tar" % (
            args.dataset, modelname, epoch, args.train_proportion)
        else:
            filename = PATH + dic + "%s/checkpoint.%s.user%.1f.item%.1f.ep%d.tp%.1f.pth.tar" % (args.dataset, modelname, args.l2u, args.l2i, epoch, args.train_proportion)'''
