...
parser.add_argument('--dataset', default="wikipedia", help='Network name')
...
'''if args.dataset == "mooc":
    print("No interaction prediction for %s" % args.dataset)
    sys.exit(0)'''
...
'''if not args.sample_length:
    ...
    dic = 'results_%s/' % args.sample_length

if args.adj:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.txt" % (args.dataset, args.method, 'adj')
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.txt" % (args.dataset, args.method, args.l2u, args.l2i,'adj')
elif args.no_zero:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.txt" % (args.dataset, args.method, 'no_zero')
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.txt" % (args.dataset, args.method, args.l2u, args.l2i,'no_zero')
elif args.no_first:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.%s.txt" % (args.dataset, args.method, 'no_first')
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.%s.txt" % (args.dataset, args.method, args.l2u, args.l2i,'no_first')
else:
    if args.l2u == 1.0 and args.l2i == 1.0:
        output_fname = dic + "interaction_prediction_%s.%s.txt" % (args.dataset, args.method)
    else:
        output_fname = dic + "interaction_prediction_%s.%s.user%.1f.item%.1f.txt" % (args.dataset, args.method, args.l2u, args.l2i,)'''
