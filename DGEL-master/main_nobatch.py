...
parser.add_argument('--bpr_coefficient', type=float, default=0.001, help='BPR is extremely bigger, e.g, 0.79. But each MSE is much small, e.g, 0.0008, so bpr_coefficient should be [0.001, 0.0005]')
...
for ep in range(args.epochs):
    ...
    # all_user_embeddings_timeseries = Variable(torch.Tensor(num_interactions, final_embedding_dim).cuda())
    # all_item_embeddings_timeseries = Variable(torch.Tensor(num_interactions, final_embedding_dim).cuda())
    ...
    for j in range(train_end_idx):
        ...
        if timestamp - tbatch_start_time > tbatch_timespan:
            ...
            for i in range(len(lib.current_tbatches_user)):
                ...
                # Record ranking by mj
                # Distance between predicted embedding and other item embeddings
                euclidean_distances = nn.PairwiseDistance()(predicted_item_embedding.repeat(num_items, 1), torch.cat([item_embeddings, item_embedding_static], dim=1).detach()).squeeze(-1)
                ...
                # record_ranking[j].append(true_item_rank)
                # Record ranking by mj
