...
def save_model(model, optimizer, args, epoch, user_embeddings, item_embeddings, train_end_idx, user_adj, item_adj,
    ...
    # mj add
    # if predicted_ranking is not None:
    #     state['predicted_ranking'] = predicted_ranking.data.cpu().numpy()
    ...
    # path = 'saved_models/%s/checkpoint_%s_epoch%d_size%d_lr%s_wd%s_seed%d_sample_length%d_bpr%s.pth.tar' % (args.dataset, args.model, epoch, args.embedding_dim, str(args.learning_rate), str(args.weight_decay), args.seed, args.sample_length, str(args.bpr_coefficient))
...
def load_model(model, optimizer, args, path):
    ...
    try:
        ...
        # mj add
        # predicted_ranking = Variable(torch.from_numpy(checkpoint['predicted_ranking']).cuda())
    ...
    return [model, optimizer, user_embeddings, item_embeddings, user_adj, item_adj,
            user_timestamp_for_adj, item_timestamp_for_adj,
            user_embeddings_time_series, item_embeddings_time_series, train_end_idx] #, predicted_ranking] # mj add
